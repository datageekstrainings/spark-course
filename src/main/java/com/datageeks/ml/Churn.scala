...
object Churn {
  ...
  case class Account(state: Option[String], len: Option[Integer], acode: Option[String],
                     intlplan: Option[String], vplan: Option[String], numvmail: Option[Double],
                     tdmins: Option[Double], tdcalls: Option[Double], tdcharge: Option[Double],
                     temins: Option[Double], tecalls: Option[Double], techarge: Option[Double],
                     tnmins: Option[Double], tncalls: Option[Double], tncharge: Option[Double],
                     timins: Option[Double], ticalls: Option[Double], ticharge: Option[Double],
                     numcs: Option[Double], churn: Option[String])
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder().appName("Spark ML App").master("local[4]").getOrCreate()

    import spark.implicits._

    val test: Dataset[Account] = spark.read.option("inferSchema", "false")
      .schema(schema).csv("C://data1/").as[Account]
    test.show()
    test.cache

    val train: Dataset[Account] = spark.read.option("inferSchema", "false")
      .schema(schema).csv("C://data/").as[Account]
    train.cache
    println(train.count)
    train.printSchema()
    train.show
    ...
    val ipindexer = new StringIndexer()
      ...
      .setOutputCol("iplanIndex").setHandleInvalid("keep")
    val labelindexer = new StringIndexer()
      .setInputCol("churn")
      .setOutputCol("label").setHandleInvalid("keep")
    ...
    predictions.collect().foreach(println(_))
